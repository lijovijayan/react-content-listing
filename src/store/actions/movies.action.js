...
export class MoviesAction {
    ...
    static fetchMovies(page) {
        return async (dispatch, getState) => {
            ...
            MoviesEffect.fetchMovies(page, searchKey).then(data => {
                ...
                dispatch(ActionUtility.createAction(MoviesAction.FETCH_MOVIES, {}, err))
            }).finally(() => {
                dispatch(ActionUtility.createAction(MoviesAction.SET_LOADER, false))
            })
        }
    }
    static searchMovies(searchKey) {
        return async (dispatch, getState) => {
            dispatch(ActionUtility.createAction(MoviesAction.SET_LOADER, true))
            MoviesEffect.fetchMovies(1, searchKey).then(data => {
                dispatch(ActionUtility.createAction(MoviesAction.FETCH_MOVIES, data))
            }).catch(err => {
                dispatch(ActionUtility.createAction(MoviesAction.FETCH_MOVIES, {}, err))
            }).finally(() => {
