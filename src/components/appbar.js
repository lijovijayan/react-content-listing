...
export function AppBar({ onChange, title }) {
    ...
    return (
        <div class={`search-bar flex flex-row items-center justify-end ${showSearch ? 'show-search' : ''}`}>
            {showSearch ? <input type="text" class="search-input flex-1 mx-3 px-3" onChange={(e) => handler(e.target.value)} placeholder="search here..." /> :
                <div className="text-xl pl-3 w-full text-left justify-start">{title}</div>
            }
            <img
